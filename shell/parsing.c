...
expand_environ_var(char *arg)
{
	if (arg && arg[0] == '$') {
		if (arg[1] == '?' && strlen(arg) == 2) {
			...
			char *buf = getenv(arg + 1);
			if (buf == NULL) {
				strcpy(arg, "");
			} else {
				if (strlen(buf) > sizeof(arg))
...
parse_exec(char *buf_cmd)
{
	...
	while (buf_cmd[idx] != END_STRING) {
		...
		// Hago este chequeo porque si tengo un caracter vacio lo tengo que descartar
		if (tok[0] != END_STRING)
			c->argv[argc++] = tok;
	}
...
parse_line(char *buf)
{
	...
	l = parse_cmd(buf);
	if (right != NULL && (strchr(right, '|') != NULL)) {
		r = parse_line(right);
	} else {
		r = parse_cmd(right);
	}
